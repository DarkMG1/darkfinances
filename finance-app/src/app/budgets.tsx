...
export default function Budgets() {
  ...
  const hasTargets = (b?.totalBudgeted ?? 0) > 0;
      ...
      {budgets.isLoading ? (
        ...
        <>
          <Text style={styles.note}>
            {hasTargets ? 'Tap a category to adjust its monthly target.' : 'No monthly targets set — tap a category to add one.'}
          </Text>
          {b.groups.map((g) => {
            ...
            return (
              <Card key={g.id} style={{ marginBottom: 12 }}>
                <View style={styles.groupHead}>
                  ...
                  <Text style={styles.groupAmt}>{hasTargets ? `${fmtPos(g.spent)} / ${fmtPos(g.budgeted)}` : fmtPos(g.spent)}</Text>
                </View>
                {g.categories.map((c) => {
                  const pct = hasTargets && c.budgeted > 0 ? (c.spent / c.budgeted) * 100 : (c.spent / groupMax) * 100;
                  return (
                    <Pressable
                      key={c.id}
                    ...
                    >
                      <View style={styles.catTop}>
                        ...
                        <Text style={styles.catAmt}>
                          {c.budgeted > 0 ? `${fmtPos(c.spent)} / ${fmtPos(c.budgeted)}` : `${fmtPos(c.spent)} · set target`}
                          {hasTargets && c.over ? ' · over' : ''}
                          {'  ›'}
                        </Text>
                      </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <Pressable style={styles.modalBg} onPress={() => setEditing(null)}>
            <Pressable style={[styles.sheet, { paddingBottom: insets.bottom + 16 }]} onPress={() => {}}>
              ...
              <Text style={styles.sheetSub}>{editing?.groupName} · spent {editing ? fmtPos(editing.spent) : ''}{month === curKey ? ' this month' : ''}</Text>
